 ...
 */
const ArticleParser = ({ url, htmlContent, onParsed, onError }) => {
    ...
    const splitIntoPassages = (text, articleId) => {
    ...
    if (isLoading) {
        return _jsx("div", { className: "article-parser-loading", children: "Extracting article content..." });
    }
    if (error) {
        return _jsxs("div", { className: "article-parser-error", children: ["Error: ", error] });
    }
