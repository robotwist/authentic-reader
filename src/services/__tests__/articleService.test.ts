...
describe('ArticleService', () => {
  ...
  describe('getArticles', () => {
    it('fetches articles from backend successfully', async () => {
      ...
      expect(fetch).toHaveBeenCalledWith(
        expect.stringContaining('/api/balanced-feed?categories=politics,technology&limit=50')
      )
    })
  ...
  describe('filterArticlesByCategories', () => {
    ...
    it('filters articles by multiple categories', () => {
      const result = articleService.filterArticlesByCategories(
        mockArticles,
        ['politics', 'technology'],
        10
      )
  ...
  describe('fetchFromBackend', () => {
    it('fetches articles from backend successfully', async () => {
      ...
      expect(result).toEqual(mockArticles)
      expect(fetch).toHaveBeenCalledWith(
        expect.stringContaining('/api/balanced-feed?categories=politics&limit=10')
      )
    })
