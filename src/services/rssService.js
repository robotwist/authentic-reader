...
function parseRssFormat(channel) {
...
function normalizeItem(item) {
    ...
    if (item.category) {
        if (Array.isArray(item.category)) {
            categories = item.category.map((c) => {
                if (typeof c === 'string')
                    return c;
                return c._ || c.$ && c.$.term || '';
            }).filter(Boolean);
...
export async function fetchFeeds(sources = DEFAULT_SOURCES) {
    try {
...
import { articlesApi, sourcesApi } from './apiService';
/**
 * Convert an API article to an RSSArticle
 */
const mapToRSSArticle = (article) => {
    if (!article) {
...
const rssService = {
    ...
    getArticlesFromUrl: async (url) => {
        ...
        try {
            ...
            try {
                const response = await fetch(`${API_BASE_URL}/api/rss?url=${encodeURIComponent(url)}`, { signal: controller.signal });
                clearTimeout(timeoutId);
                if (!response.ok) {
                ...
                return data.items.map((item) => mapToRSSArticle({
                    ...item,
