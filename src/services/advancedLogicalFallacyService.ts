...
export interface LogicalFallacy {
  ...
  category: 'formal' | 'informal' | 'statistical' | 'causal' | 'presumption';
  explanation: string;
...
export interface FallacyAnalysisResult {
  ...
  overallScore: number; // 0-100, lower is better (fewer fallacies)
  categories: {
    [key: string]: number; // Count by category
  };
...
class AdvancedLogicalFallacyService {
  private fallacyPatterns: Map<string, {
    patterns: RegExp[];
    keywords: string[];
    contextClues: string[];
    severity: LogicalFallacy['severity'];
  ...
  private initializeFallacyPatterns() {
  ...
  private detectFallaciesInSentence(
    sentence: string, 
    ...
    for (const [fallacyType, config] of this.fallacyPatterns.entries()) {
      ...
      const contextMatches = config.contextClues.some(clue => 
        sentence.toLowerCase().includes(clue.toLowerCase())
      );

      if (patternMatches || (keywordMatches && contextMatches)) {
        const confidence = this.calculateConfidence(
          patternMatches, keywordMatches, contextMatches, sentence, config
        );
  ...
  private detectStructuralFallacies(
    text: string, 
    metadata?: { title?: string; source?: string; author?: string; context?: string; }
  ): LogicalFallacy[] {
    const fallacies: LogicalFallacy[] = [];
  ...
  private calculateConfidence(
    ...
    if (sentenceLength > 200) confidence -= 0.1; // Longer sentences might be less clear-cut
    if (sentenceLength < 50) confidence -= 0.2; // Very short sentences might be false positives
  ...
  private deduplicateFallacies(fallacies: LogicalFallacy[]): LogicalFallacy[] {
    const seen = new Map<string, LogicalFallacy>();
  ...
  private categorizeFallacies(fallacies: LogicalFallacy[]): { [key: string]: number } {
  ...
  private calculateOverallScore(fallacies: LogicalFallacy[], textLength: number): number {
    if (fallacies.length === 0) return 100;
  ...
  private generateSummary(fallacies: LogicalFallacy[], overallScore: number): string {
    if (fallacies.length === 0) {
    ...
    if (severityDistribution.critical) severityParts.push(`${severityDistribution.critical} critical`);
    if (severityDistribution.high) severityParts.push(`${severityDistribution.high} high`);
    if (severityDistribution.medium) severityParts.push(`${severityDistribution.medium} medium`);
    if (severityDistribution.low) severityParts.push(`${severityDistribution.low} low`);
  ...
  // Helper methods
  private getFallacyName(type: string): string {
  ...
  private extractExamples(sentence: string, config: any): string[] {
  ...
  private getContext(sentence: string, paragraphs: string[], paragraphIndex: number): string {
  ...
  private generateCounterargument(fallacyType: string, sentence: string): string {
    const counterarguments: { [key: string]: string } = {
  ...
  private detectCherryPicking(text: string): boolean {
    const indicators = [
  ...
  private detectLoadedQuestions(text: string): boolean {
    const patterns = [
