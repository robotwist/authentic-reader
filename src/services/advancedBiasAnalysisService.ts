...
export interface BiasIndicator {
  type: 'political' | 'emotional' | 'cognitive' | 'linguistic' | 'statistical' | 'source';
  name: string;
  description: string;
...
export interface BiasScore {
  overall: number; // 0-100, 50 is neutral
  political: {
    leftRight: number; // 0-100, 0=far-left, 50=center, 100=far-right
    confidence: number;
    indicators: string[];
  };
  emotional: {
...
export interface AdvancedBiasAnalysisResult {
...
class AdvancedBiasAnalysisService {
  private politicalKeywords: Map<string, { lean: 'left' | 'right'; weight: number }>;
  private emotionalKeywords: Map<string, { emotion: keyof BiasScore['emotional']; intensity: number }>;
  private cognitivePatterns: Map<string, { bias: string; patterns: RegExp[]; weight: number }>;
  private loadedLanguagePatterns: RegExp[];
  ...
  ): Promise<AdvancedBiasAnalysisResult> {
  ...
  private analyzePoliticalBias(text: string): BiasIndicator[] {
    const indicators: BiasIndicator[] = [];
  ...
  private analyzeEmotionalBias(text: string): BiasIndicator[] {
    const indicators: BiasIndicator[] = [];
    const lowerText = text.toLowerCase();
  ...
  private analyzeCognitiveBias(text: string): BiasIndicator[] {
    const indicators: BiasIndicator[] = [];
  ...
  private analyzeLinguisticBias(text: string): BiasIndicator[] {
    const indicators: BiasIndicator[] = [];
  ...
  private analyzeStatisticalBias(text: string): BiasIndicator[] {
    const indicators: BiasIndicator[] = [];
    ...
    if (statistics && statistics.length > 0) {
      ...
      const hasCherryPickingClues = cherryPickingClues.some(clue => 
  ...
  private analyzeSourceBias(metadata?: {
    title?: string;
    source?: string;
    author?: string;
    publishDate?: string;
    ...
    if (!metadata?.source) return indicators;

    // Known biased sources (this would be expanded with a comprehensive database)
    const knownBiases: { [key: string]: { lean: string; severity: 'low' | 'medium' | 'high' } } = {
  ...
  private calculateBiasScores(text: string, indicators: BiasIndicator[]): BiasScore {
    ...
    const overall = (
      Math.abs(leftRightScore - 50) + // Political deviation from center
      emotional.overall +
      cognitive.overall +
      linguistic.overall
    ) / 4;
  ...
  private calculateEmotionalScore(emotion: string, indicators: BiasIndicator[]): number {
    const relevantIndicators = indicators.filter(i => 
      i.name.toLowerCase().includes(emotion.toLowerCase())
    );
  ...
  private calculateCognitiveScore(biasName: string, indicators: BiasIndicator[]): number {
  ...
  private calculateLinguisticScore(type: string, indicators: BiasIndicator[]): number {
  ...
  private calculateOverallConfidence(indicators: BiasIndicator[]): number {
    if (indicators.length === 0) return 0.5;
    return indicators.reduce((sum, i) => sum + i.confidence, 0) / indicators.length;
  }

  private generateSummary(indicators: BiasIndicator[], scores: BiasScore) {
