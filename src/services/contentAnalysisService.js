...
import { analyzeEmotions } from './emotionAnalysisService';
import { aiAnalysisService } from './aiAnalysisService';
import { analyzeManipulation } from './darkPatternService';
/**
 * Types of logical fallacies that can be detected
 */
export var LogicalFallacyType;
(function (LogicalFallacyType) {
    LogicalFallacyType["AD_HOMINEM"] = "AD_HOMINEM";
    LogicalFallacyType["STRAW_MAN"] = "STRAW_MAN";
    LogicalFallacyType["APPEAL_TO_EMOTION"] = "APPEAL_TO_EMOTION";
    LogicalFallacyType["FALSE_DILEMMA"] = "FALSE_DILEMMA";
    LogicalFallacyType["SLIPPERY_SLOPE"] = "SLIPPERY_SLOPE";
    LogicalFallacyType["HASTY_GENERALIZATION"] = "HASTY_GENERALIZATION";
    LogicalFallacyType["CIRCULAR_REASONING"] = "CIRCULAR_REASONING";
    LogicalFallacyType["APPEAL_TO_AUTHORITY"] = "APPEAL_TO_AUTHORITY";
    LogicalFallacyType["BANDWAGON"] = "BANDWAGON";
    LogicalFallacyType["RED_HERRING"] = "RED_HERRING";
    LogicalFallacyType["APPEAL_TO_IGNORANCE"] = "APPEAL_TO_IGNORANCE";
    LogicalFallacyType["TU_QUOQUE"] = "TU_QUOQUE";
    LogicalFallacyType["ANECDOTAL"] = "ANECDOTAL";
    LogicalFallacyType["LOADED_QUESTION"] = "LOADED_QUESTION";
    LogicalFallacyType["NO_TRUE_SCOTSMAN"] = "NO_TRUE_SCOTSMAN";
    LogicalFallacyType["TEXAS_SHARPSHOOTER"] = "TEXAS_SHARPSHOOTER";
    LogicalFallacyType["MIDDLE_GROUND"] = "MIDDLE_GROUND";
    LogicalFallacyType["GAMBLER_FALLACY"] = "GAMBLER_FALLACY";
    LogicalFallacyType["APPEAL_TO_NATURE"] = "APPEAL_TO_NATURE";
    LogicalFallacyType["COMPOSITION_DIVISION"] = "COMPOSITION_DIVISION";
})(LogicalFallacyType || (LogicalFallacyType = {}));
/**
 * Types of political bias
 */
export var BiasType;
(function (BiasType) {
    BiasType["LEFT_EXTREME"] = "LEFT_EXTREME";
    BiasType["LEFT_STRONG"] = "LEFT_STRONG";
    BiasType["LEFT_MODERATE"] = "LEFT_MODERATE";
    BiasType["LEFT_SLIGHT"] = "LEFT_SLIGHT";
    BiasType["CENTER"] = "CENTER";
    BiasType["RIGHT_SLIGHT"] = "RIGHT_SLIGHT";
    BiasType["RIGHT_MODERATE"] = "RIGHT_MODERATE";
    BiasType["RIGHT_STRONG"] = "RIGHT_STRONG";
    BiasType["RIGHT_EXTREME"] = "RIGHT_EXTREME";
})(BiasType || (BiasType = {}));
 ...
 */
export function detectLogicalFallacies(text) {
    const fallacies = [];
                ...
                if (pattern.test(sentence)) {
                    ...
                    if (match) {
                        fallacies.push({
                            ...
                            explanation: getFallacyExplanation(fallacyType),
                            excerpt: sentence,
                            startIndex,
                            endIndex
                        });
 ...
 */
function calculateFallacyConfidence(sentence, pattern) {
...
function getFallacyExplanation(type) {
    const explanations = {
        [LogicalFallacyType.APPEAL_TO_AUTHORITY]: "Appeal to Authority: Using an authority figure's opinion to support an argument, rather than evidence or logical reasoning.",
        [LogicalFallacyType.APPEAL_TO_EMOTION]: "Appeal to Emotion: Manipulating emotions to win an argument rather than using logical reasoning.",
        [LogicalFallacyType.STRAW_MAN]: "Straw Man: Misrepresenting someone's argument to make it easier to attack.",
        [LogicalFallacyType.AD_HOMINEM]: "Ad Hominem: Attacking the person instead of addressing their argument.",
        [LogicalFallacyType.RED_HERRING]: "Red Herring: Introducing an irrelevant topic to divert attention from the original issue.",
        [LogicalFallacyType.FALSE_DICHOTOMY]: "False Dichotomy: Presenting only two options when others exist.",
        [LogicalFallacyType.APPEAL_TO_IGNORANCE]: "Appeal to Ignorance: Claiming something is true because it hasn't been proven false.",
        [LogicalFallacyType.APPEAL_TO_COMMON_SENSE]: "Appeal to Common Sense: Arguing that a claim must be true because it seems like 'common sense'.",
        [LogicalFallacyType.CIRCULAR_REASONING]: "Circular Reasoning: Making an argument where the conclusion is included in the premise.",
        [LogicalFallacyType.POST_HOC]: "Post Hoc: Assuming that because one event followed another, the first caused the second.",
        [LogicalFallacyType.FALSE_EQUIVALENCE]: "False Equivalence: Comparing two things as if they're equivalent when they're not.",
        [LogicalFallacyType.BANDWAGON]: "Bandwagon: Appealing to popularity as evidence for a claim's validity.",
        [LogicalFallacyType.TU_QUOQUE]: "Tu Quoque: Avoiding criticism by turning it back on the accuser.",
        [LogicalFallacyType.LOADED_QUESTION]: "Loaded Question: Asking a question that contains an assumption that the person answering would find difficult to challenge.",
        [LogicalFallacyType.SLIPPERY_SLOPE]: "Slippery Slope: Arguing that a small first step will inevitably lead to a chain of related events culminating in some significant (usually negative) effect.",
        [LogicalFallacyType.HISTORICAL_ANALOGY]: "Faulty Historical Analogy: Assuming that because two events share some similarities, the outcome or characteristics of one will apply to the other, often ignoring key differences in context."
    };
 ...
 */
export async function analyzeBias(text) {
    try {
        // Check cache first
 ...
 */
function extractBiasIndicators(text, leaning) {
    const indicators = [];
 ...
 */
function fallbackBiasAnalysis(text) {
    const lowerText = text.toLowerCase();
    let leftScore = 0;
    let rightScore = 0;
    const leftKeywordsFound = [];
    const rightKeywordsFound = [];
    ...
    if (biasDirection === 'left') {
        if (biasStrength > 0.75)
            biasType = BiasType.LEFT_EXTREME;
        else if (biasStrength > 0.5)
            biasType = BiasType.LEFT_STRONG;
        else if (biasStrength > 0.25)
            biasType = BiasType.LEFT_MODERATE;
        else
            biasType = BiasType.LEFT_SLIGHT;
    }
 ...
 */
function getBiasStrengthLabel(strength) {
    if (strength > 0.75)
        return 'very strong';
    if (strength > 0.5)
        return 'strong';
 ...
 */
export async function extractMetadata(html, text) {
    try {
        // Validate input
 ...
 */
async function performZeroShotAnalysis(text, candidateLabels) {
    if (!NLP_SERVICE_URL) {
...
export async function analyzeContent(content) {
    try {
        ...
        const qualityScore = calculateQualityScore(plainText, logicalFallacies, biasAnalysis, manipulationAnalysis ? manipulationAnalysis.score : 0.1);
        ...
        const addLoadingIndicators = (result) => {
            result.loadingState = {
 ...
 */
export function getSortFunction(option) {
    switch (option) {
 ...
 */
function splitIntoAnalysisChunks(text, maxChunkLength = 500) {
 ...
 */
function combineIntoChunks(segments, maxChunkLength) {
    const chunks = [];
    let currentChunk = '';
 ...
 */
async function analyzeTextChunksForFallacies(chunks, fallacyLabels) {
    // If no chunks, return empty array
    if (!chunks.length)
        return [];
    try {
        // Use Promise.all to process all chunks in parallel
        const results = await Promise.all(chunks.map(chunk => performZeroShotAnalysis(chunk, fallacyLabels)));
 ...
 */
function generateEmotionalAppealsModel(text, emotionAnalysis) {
 ...
 */
function generateSentimentAnalysis(text, sentiment) {
    ...
    aspectPatterns.forEach(aspect => {
        ...
        sentences.forEach(sentence => {
            ...
            const containsAspect = aspect.patterns.some(pattern => lowerSentence.includes(pattern));
            if (containsAspect) {
                matchCount++;
 ...
 */
function calculateQualityScore(text, logicalFallacies, biasAnalysis, manipulationScore) {
 ...
 */
async function cachedEmotionAnalysis(text) {
    const cacheKey = cache.getKey(text);
 ...
 */
async function cachedSentimentAnalysis(text) {
    const cacheKey = cache.getKey(text);
    // Check if we have a cached result
 ...
 */
async function cachedEntityExtraction(text) {
    const cacheKey = cache.getKey(text);
    // Check if we have a cached result
 ...
 */
async function cachedSummaryGeneration(text) {
    const cacheKey = cache.getKey(text);
    // Check if we have a cached result
