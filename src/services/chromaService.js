...
class ChromaService {
     ...
     */
    async storeArticleEmbedding(articleId, embedding, metadata) {
        if (!this.isConfigured) {
     ...
     */
    async getSimilarArticles(embedding, limit = 5) {
        if (!this.isConfigured) {
        ...
        try {
            ...
            // Format the results
            const articles = result.metadatas[0].map((metadata, i) => {
                return {
                    id: result.ids[0][i],
    ...
    async getArticleFeedback(articleId) {
        if (!this.isConfigured) {
            ...
            const localFeedback = JSON.parse(localStorage.getItem('feedback') || '[]');
            return localFeedback.filter((item) => item.articleId === articleId);
        }
        try {
            ...
            const result = await response.json();
            return result.metadatas.map((metadata) => metadata);
        }
        catch (error) {
