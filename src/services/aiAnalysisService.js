...
export class AIAnalysisService {
    ...
    async analyzeBiasWithLlama(text) {
        try {
...
Provide specific examples from the text and explain how each type of bias might influence reader perception.`;
    ...
    async analyzeSentimentWithLlama(text) {
        try {
...
Provide specific examples from the text and explain how linguistic choices create emotional effects.`;
    ...
    async extractEntitiesWithLlama(text) {
        try {
...
Text: "${text}"`;
    ...
    async analyzeCredibilityWithLlama(text) {
        try {
...
Text: "${text}"`;
    ...
    async analyzeBiasFallback(text) {
        ...
        Object.entries(biasIndicators).forEach(([type, indicators]) => {
            const foundIndicators = indicators.filter(indicator => text.toLowerCase().includes(indicator.toLowerCase()));
    ...
    extractBiasedPhrases(text, biasWords) {
        const phrases = [];
    ...
    async extractEntitiesFallback(text) {
    ...
    async analyzeCredibilityFallback(text) {
    ...
    async parseLlamaResponse(response, article) {
        try {
