...
import { toRSSArticle } from '../types';
import { analyzeContent } from '../services/contentAnalysisService';
import { useAuth } from '../contexts/AuthContext';
import { saveArticles, getArticles, saveSources, getAllSources, markArticleAsRead, markArticleAsSaved, saveArticleAnalysis, getArticleAnalysis } from '../services/storageService';
...
export function useArticles() {
    ...
    const refreshArticles = useCallback(async () => {
        ...
        try {
            ...
            const articlesBySource = await Promise.race([
                Promise.allSettled(articlePromises).then(results => results.map(result => result.status === 'fulfilled' ? result.value : [])),
                timeoutPromise
            ]);
                    ...
                    if (apiSources && apiSources.length > 0 && isMounted) {
                        ...
                        if (isLoggedIn) {
                            try {
                                ...
                                const userSourcesTimeout = new Promise((_, reject) => {
    ...
    const analyzeArticle = useCallback(async (article) => {
        ...
        try {
            ...
            let contentToAnalyze = undefined;
    ...
    const getAnalysis = useCallback(async (articleGuid) => {
        try {
    ...
    const markAsRead = useCallback(async (guid) => {
        try {
            ...
            // Update local state
            const updatedArticles = articles.map(article => compareGuids(article.guid, guid) ? { ...article, isRead: true } : article);
            setArticles(updatedArticles);
            setFilteredArticles(prevFiltered => prevFiltered.map(article => compareGuids(article.guid, guid) ? { ...article, isRead: true } : article));
        }
        catch (e) {
    ...
    const markAsSaved = useCallback(async (guid) => {
        try {
            // If the user is logged in, use the API
            if (isLoggedIn) {
            ...
            // Update local state
            const updatedArticles = articles.map(article => compareGuids(article.guid, guid) ? { ...article, isSaved: true } : article);
            setArticles(updatedArticles);
            setFilteredArticles(prevFiltered => prevFiltered.map(article => compareGuids(article.guid, guid) ? { ...article, isSaved: true } : article));
        }
        catch (e) {
    ...
    const filterArticles = useCallback(async (filters) => {
        try {
            ...
            if (filters.sources && filters.sources.length > 0) {
                filteredResults = filteredResults.filter(article => {
                    ...
                    if (typeof article.source === 'string') {
                        ...
                        return filters.sources.includes(article.source.name);
                    }
                    return false;
                });
            }
            ...
            if (filters.categories && filters.categories.length > 0) {
                filteredResults = filteredResults.filter(article => article.categories && article.categories.some(category => filters.categories.includes(category.toLowerCase())));
            }
            // Apply search query if exists
            if (searchQuery) {
                filteredResults = filteredResults.filter(article => article.title.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    const searchArticles = useCallback((query) => {
        ...
        const searchedArticles = filteredArticles.filter(article => article.title.toLowerCase().includes(query.toLowerCase()) ||
    ...
    const sortArticles = useCallback((sortBy) => {
        ...
        const sortOption = sortBy; // Type assertion, consider proper validation
...
function getSortFunction(option) {
    ...
    switch (option) {
        case SortOption.NEWEST_FIRST:
            return (a, b) => new Date(b.publishDate || 0).getTime() - new Date(a.publishDate || 0).getTime();
